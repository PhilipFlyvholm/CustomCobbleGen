...
public class PlayerData {
	...
	public PlayerData(UUID uuid, SelectedTiers selectedTiers, List<Tier> purchasedTiers) {
		...
		if(selectedTiers == null) {
			plugin.error("Failed to load player data: selectedTiers is null for uuid " + uuid.toString());
			return;
		}
		if(purchasedTiers == null) {
			plugin.error("Failed to load player data: purchasedTiers is null for uuid " + uuid.toString());
			return;
		}
		this.uuid = uuid;
