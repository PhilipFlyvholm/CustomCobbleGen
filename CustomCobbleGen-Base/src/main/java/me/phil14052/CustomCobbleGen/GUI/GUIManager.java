...
public class GUIManager {
	...
	public Icon getNoPermissionsIcon(String permission) {
		return new Icon(
				new ItemLib(
						...
						Collections.singletonList(Lang.GUI_ADMIN_NO_PERMISSION_LORE.toString(permission))
					).create()
				);
	}
	...
	public class MainGUI {
		...
		public MainGUI(Player p){
			...
			if(tiers == null) {
				p.sendMessage(Lang.PREFIX.toString() + Lang.NO_TIERS_DEFINED);
				failedLoad = true;
				return;
			}
			int i = 0; //Current pos
	...
	public class ConfirmGUI {
		...
		public ConfirmGUI(Player p, Tier tier){
			...
			if(buyItem == null) buyItem = new ItemStack(Material.LIME_DYE);
	...
	public class UpgradeGUI {
		private final int GUISize = 9;
		private final CustomHolder ch = new CustomHolder(GUISize, Lang.GUI_UPGRADE_TITLE.toString());
		private final Player player;
						...
						if(tm.hasPlayerPurchasedLevel(p, tier)) {
							...
							if(tm.canPlayerBuyTier(p, tier)) {
								if(tm.purchaseTier(p, tier)) {
									...
									p.sendMessage(Lang.PREFIX + Lang.TIER_CHANGED.toString(tier));
									if(Setting.ISLANDS_USEPERISLANDUNLOCKEDGENERATORS.getBoolean() && Setting.ISLANDS_SENDMESSAGESTOTEAM.getBoolean()) {
										plugin.getIslandHook().sendMessageToIslandMembers(Lang.PREFIX + Lang.TIER_UPGRADED_BY_TEAM.toString(p, tier),
												p.getUniqueId(),
												true);
									}
									p.closeInventory();
	...
	public class CreateTierGUI{
		private final int GUISize = 27;
		private final CustomHolder ch = new CustomHolder(GUISize, Lang.GUI_CREATE_TITLE.toString());
		private final Player player;
		...
		public CreateTierGUI(Player p, Tier createdTier) {
			...
			List<String> classIconLore = Lang.GUI_CREATE_CLASS_LORE.toStringList(tierCreated 
					&& (createdTier != null && createdTier.getTierClass() != null)
			...
			classIcon.addClickAction(new ClickAction() {
				...
				public void execute(Player p) {
					...
					p.sendMessage(Lang.PREFIX + Lang.CHATINPUT_INFO_CANCEL.toString("CANCEL"));
					p.closeInventory();
				}
			...
			List<String> levelIconLore = Lang.GUI_CREATE_LEVEL_LORE.toStringList(tierCreated 
					&& createdTier != null && createdTier.getLevel() >= 0 
			...
			List<String> nameIconLore = Lang.GUI_CREATE_NAME_LORE.toStringList(tierCreated 
					&& createdTier != null && createdTier.getName() != null 
			...
			boolean iconMaterialSet = tierCreated && createdTier != null && createdTier.getIcon() != null && !createdTier.getIcon().getType().equals(Material.AIR);
			List<String> materialIconLore = Lang.GUI_CREATE_ICON_LORE.toStringList(iconMaterialSet && createdTier != null
						? "\"" + createdTier.getIcon().getType().name() + "\"" 
						: Lang.GUI_CREATE_EMPTY.toString());
			Icon materialIcon = new Icon(new ItemLib(createdTier != null && iconMaterialSet ? createdTier.getIcon().getType() : paperMaterial, 1, (short) 0, Lang.GUI_CREATE_ICON.toString(p), materialIconLore).create());
			...
			if(tierCreated && createdTier != null && createdTier.getResults() != null && !createdTier.getResults().isEmpty()) {
			...
			List<String> requirementsIconLore = Lang.GUI_CREATE_REQUIREMENTS_LORE.toStringList(tierCreated && createdTier != null && createdTier.getRequirements() != null ? createdTier.getRequirements().size() + "" : "0");
			...
			if(tierCreated && createdTier != null && createdTier.hasDescription()) {
	...
	public class PlayerSelectGUI {
		...
		private final CustomHolder ch = new CustomHolder(guiSize, Lang.GUI_SELECT_TITLE.toString());
		private final Player player;
	...
	public class TierSelectGUI {
		...
		int tiersSize = tm.getTiersSize();
		int guiSize = getGUISize(tiers, false);
		private final CustomHolder ch = new CustomHolder(guiSize, Lang.GUI_SELECT_TIER_TITLE.toString());
		private final Player player;
		private boolean failedLoad = false;
			...
			for(String tierClass : tiers.keySet()) {
				...
				for(Tier tier : classTiers) {
					...
					if(currentRow == rows) {
						...
						if(j == firstItemInRow) {
							int emptySpacesAtStart = (int) Math.ceil((4.5 -(itemsInRow/2)));
